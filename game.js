...
// Initialize players with a default last direction
const player1 = {
  x: 100,
  y: 0,
  width: 40,
...
function drawTopStatus() {
  ...
  // Health bar (red with white outline)
  ctx.fillStyle = "red";
  ...
  // Shield bar (blue gradient with white outline)
  ...
  ctx.fillText(p1Name, leftX, topY + barHeight*2 + 25);
  ...
  ctx.fillStyle = "red";
  ctx.font = "bold 16px Arial";
  ctx.fillText(p2Name, rightX + barWidth, topY + barHeight*2 + 25);
  ctx.textAlign = "left";
}

function drawControls() {
  // Enhanced 3D effect for control boxes
  ...
  ctx.lineWidth = 2;
  ctx.stroke();
  // Draw a highlight line at the top for 3D effect
  ctx.beginPath();
  ctx.moveTo(leftX + radius, leftY + 2);
  ctx.lineTo(leftX + boxWidth - radius, leftY + 2);
  ...
  ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(rightX + radius, rightY + 2);
  ctx.lineTo(rightX + boxWidth - radius, rightY + 2);
  ctx.strokeStyle = "rgba(255,255,255,0.6)";
  ctx.lineWidth = 2;
  ctx.stroke();
  ctx.restore();
...
function shootBullet(player, owner) {
  ...
  // Determine bullet spawn and velocity based on player's lastDir
